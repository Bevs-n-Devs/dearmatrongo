...
func DeleteGDPRData(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		...
		http.Redirect(w, r, "/uk/gdpr", http.StatusSeeOther)
		return
	}
	...
	if name == "" || email == "" || incidentDate == "" || facilityType == "" || sendDataEmail == "" {
	...
	match, err := database.CheckGDPRData(name, email, incidentDate, facilityType)
	if err != nil {
	...
	getData, err := database.GDPRDearMatronFullReport(name, email, incidentDate, facilityType)
	if err != nil {
	...
	err = database.DeleteGDPRData(name, email, incidentDate, facilityType)
	if err != nil {
